...
def fuel_consumption():
    ...
    for bus in buses:
        # Calculate last week's consumption
        last_week_records = Fuel.query.filter(
            Fuel.bus_id == bus.id,
            Fuel.date >= one_week_ago
        ).all()
        bus.last_week_consumption = sum(record.fuel_amount for record in last_week_records)
        
        # Calculate total mileage (example calculation)
        total_fuel = sum(record.fuel_amount for record in bus.fuel_records)
        bus.total_mileage = total_fuel * 10  # Assuming 10km per liter, adjust as needed
...
def add_fuel_consumption():
    ...
    try:
        ...
        if not all([bus_id, fuel_amount, date_str]):
        ...
        flash('Fuel consumption record added successfully')
        return redirect(url_for('fuel_consumption'))
